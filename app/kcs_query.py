...
# In[126]:
def query_generator(query_terms,query_date):
    ...
    query_df = processed_df[processed_df['date_ps'] >= query_date]
    query_terms = query_terms
    search = '|'.join(query_terms.split())


    # In[127]:

    query_score = processed_df['article_ps'].str.count(search) + processed_df['title_ps'].str.count(search)


    # In[128]:

    query_df['query_score'] = query_score


    # In[129]:

    query_df = query_df[query_df['query_score']>0].sort_values(by= 'query_score',ascending=False).reset_index(drop=True)
    if query_df.shape[0] > 1000:
        query_df = query_df.loc[0:999,:]


    # In[130]:

    del query_df['query_score']
    ...
    query_tfidf = tfidftransformer.fit_transform(query_df.iloc[:,9:])
    query_tfidf = query_tfidf.A
    ...
    ##Remove tfidf vectors for a clean dataframe
    queried_df = query_df[['title_ps','author_ps','date_ps','sent_score','article_length','summary_ps','group_cluster']]
    queried_df.columns = ['Title','Author','Date','Sentiment','Length','Summary','Group']

    # In[146]:

    queried_df.to_pickle('queried_df.pkl')
    queried_df.to_csv('queried_df.csv')
